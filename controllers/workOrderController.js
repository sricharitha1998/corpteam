...
supplyTotalRate: req.body.supplyTotalRate,
                        serviceTotalRate: req.body.serviceTotalRate,
    ...
    UpdateStatus: async (req, res) => {
        ...
        switch (methods) {
            case "POST":
                try {
                    await workOrderModel.findByIdAndUpdate({ _id: req.body.id }, { status: req.body.status }, { upsert: true }).then(async function (doc, err) {
                       
                        if (err) {
                            console.log("err", err);
                            return next(err);
                        }
			await userModel.findOne({ _id: doc?.vendor_id }).then(async function (user, err) {
                            if (err) {
                                console.log("err", err);
                                return next(err);
                            }
                        
                        let HtmlMsg = `<div><p><b>${user?.username} accepted the work order successfully: <br /> Work Order:</b> ${doc.workOrderNumber}</p></div>`;
                            let subject="Work Order Status";
